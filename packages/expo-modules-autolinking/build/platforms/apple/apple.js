...
async function findPodspecFiles(revision) {
  ...
  if (configPodspecPaths && configPodspecPaths.length) {
    ...
    return await (0, utils_1.listFilesInDirectories)(revision.path, (basename) =>
...
async function generatePackageListFileContentAsync(modules, className, entitlements) {
  ...
  const debugOnlyModulesClassNames = []
    ...
    .filter(Boolean);
  const appDelegateSubscribers = [].concat(
    ...modulesToImport.map((module) => module.appDelegateSubscribers)
  );
  const debugOnlyAppDelegateSubscribers = [].concat(
...
async function parseEntitlementsAsync(entitlementPath) {
  ...
  const { stdout } = await (0, spawn_async_1.default)('plutil', [
    '-convert',
    'json',
    '-o',
    '-',
