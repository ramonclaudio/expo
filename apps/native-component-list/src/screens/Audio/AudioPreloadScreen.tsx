...
function SoundEffectButtons() {
  ...
  const playSfx = (player: typeof player1, status: typeof status1) => {
    if (status.playing) {
      player.seekTo(0);
    } else {
...
function PreloadCacheInfo() {
  ...
  const refresh = () => setSources(getPreloadedSources());
  ...
  return (
    <View>
      <ListButton title="Refresh Cache Info" onPress={refresh} />
      <Text style={styles.hint}>
        {sources.length === 0
          ? 'Cache is empty (sources were consumed by players or cleared).'
          : `${sources.length} source(s) in cache:`}
      </Text>
      {sources.map((uri) => (
